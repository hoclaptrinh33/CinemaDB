...
function query() {
    router.get(route('moderate.reviews.index'), {
        search: search.value,
...
function destroy(id) {
    router.delete(route('moderate.reviews.destroy', id), {
        preserveScroll: true,
...
function updateStatus(id, status) {
    router.patch(route('moderate.reviews.status', id), { status }, { preserveScroll: true });
}
...
<template>
    <div class="space-y-4">
        <div class="flex items-center justify-between">
            <h1 class="font-display font-bold text-2xl text-[var(--color-text-primary)]">Moderate Reviews</h1>
        </div>
        ...
        <div class="flex flex-wrap gap-3">
            ...
            <select v-model="statusFilter" class="input-base w-36" @change="query">
                <option value="">Tất cả</option>
                <option value="VISIBLE">Visible</option>
                <option value="HIDDEN">Hidden</option>
            </select>
            <p class="text-[var(--color-text-muted)] text-sm self-center">{{ reviews.total }} reviews</p>
        </div>
            ...
            <table class="data-table">
                ...
                <tbody>
                    <tr v-for="r in reviews.data" :key="r.review_id">
                        ...
                        <td><Badge :variant="r.moderation_status === 'VISIBLE' ? 'success' : 'danger'">{{ r.moderation_status }}</Badge></td>
