...
<template>
    ...
    <div class="flex items-center justify-between">
        <h1 class="font-display font-bold text-2xl text-[var(--color-text-primary)]">Audit Log</h1>
    </div>
        <div class="flex items-center gap-2">
            ...
            <div class="w-40 shrink-0">
                <select v-model="actionFilter" class="input-base" @change="query">
            ...
            <p class="text-[var(--color-text-muted)] text-sm ml-auto shrink-0">{{ logs.total }} bản ghi</p>
        </div>
            ...
            <table class="data-table">
                ...
                <tbody>
                    <tr v-for="log in logs.data" :key="log.id">
                        ...
                        <td class="text-sm text-[var(--color-text-muted)] max-w-[180px]">
                            <span class="truncate block">{{ log.title_name ?? `#${log.title_id}` }}</span>
                        </td>
                        <td class="max-w-xs">
